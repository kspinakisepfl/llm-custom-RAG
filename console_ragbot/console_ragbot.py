...
def search_engine(query: str, faiss_index: faiss.Index, db_path: str, k: int = 5) -> List[Tuple[str, float, str, int]]:
    ...
    for i, distance in zip(indices[0], distances[0]):
        ...
        chunkid, chunk_content, sourcedoc,startpage = cursor.fetchone()
...
def run_agent_conversation() -> None:
    ...
    while True:
        ...
        ########################################################################
        response = agent_executor.invoke({"input": user_input,"chat_history":memory.load_memory_variables({})["chat_history"]})
        memory.save_context({"input": response['input']}, {"output":response['output']})
        ...
        for id in ids:
            ...
            ####################################################################
            response["output"] = response["output"].replace(f"[[{id}]]","Source: " + str(chunk_content[0]) + "\n" + "Page: " + str(chunk_content[1]))
...
if __name__ == "__main__":
    ...
    print("Now reading and chunking:")
